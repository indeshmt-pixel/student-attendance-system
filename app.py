...
def students():
    ...
    if request.method == 'POST':
        ...
        elif email and not re.fullmatch(r'[A-Za-z0-9.!#$%&\'*+/=?^_`{|}~-]+@[A-Za-z0-9-]+(?:\\.[A-Za-z0-9-]+)+', email):
            flash('Please enter a valid Gmail/email address.','error')
        elif c.execute('SELECT 1 FROM students WHERE lower(trim(roll_no))=lower(trim(?))',(roll_no,)).fetchone():
            flash('This roll number already exists. Student was not added again.','error')
        elif email and c.execute('SELECT 1 FROM students WHERE lower(trim(email))=lower(trim(?))',(email,)).fetchone():
            flash('This email is already registered with another student.','error')
        else:
            c.execute(
                'INSERT INTO students(name,roll_no,course,semester,email) VALUES(?,?,?,?,?)',
                (name,roll_no,course,semester,email)
            )
            c.commit()
        ...
        rows = c.execute(
            '''SELECT * FROM students
               WHERE name LIKE ? OR roll_no LIKE ? OR course LIKE ?
                  OR semester LIKE ? OR email LIKE ?
               ORDER BY id DESC''',
